...
grep -Fq -- 'const focusRow = (...ids) =>' "$CLIENT"
grep -Fq -- 'onComplete: () => focusRow("sync")' "$CLIENT"
grep -Fq -- 'ariaLabel: `应用预览 ${preview.preview_id}`' "$CLIENT"
grep -Fq -- 'ariaLabel: `丢弃预览 ${preview.preview_id}`' "$CLIENT"
